...
func (c *Cipher) isCorrectData(data []byte) error {
...
func generateAESIGEv2(msgKey, authKey []byte, decode bool) (aesKey, aesIv []byte) { //nolint: deadcode wait for it
	...
	sha1PartA := sha1.Sum(tA)
	sha1PartB := sha1.Sum(tB)
	sha1PartC := sha1.Sum(tC)
	sha1PartD := sha1.Sum(tD)
...
func generateAESIGE(msg_key, auth_key []byte, decode bool) ([]byte, []byte) {
	...
	aes_key := make([]byte, 0, 32) //nolint
	aes_iv := make([]byte, 0, 32)  //nolint
	t_a := make([]byte, 0, 48)     //nolint
	t_b := make([]byte, 0, 48)     //nolint
	t_c := make([]byte, 0, 48)     //nolint
	t_d := make([]byte, 0, 48)     //nolint
